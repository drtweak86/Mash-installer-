...
fn level_filter(config: &installer_model::config::LoggingConfig) -> LevelFilter {
...
fn build_env_filter(config: &installer_model::config::LoggingConfig, verbose: bool) -> EnvFilter {
...
pub fn init(config: &installer_model::config::LoggingConfig, verbose: bool) -> Result<()> {
    let log_file = config
        .file
        .clone()
...
mod tests {
    ...
    fn json_format_records_structured_entries() -> Result<()> {
        ...
        let config = installer_model::config::LoggingConfig {
            level: "info".into(),
            format: installer_model::config::LogFormat::Json,
    ...
    fn level_filter_filters_lower_levels() -> Result<()> {
        ...
        let config = installer_model::config::LoggingConfig {
            level: "warn".into(),
            format: installer_model::config::LogFormat::Human,
            file: Some(log_path.clone()),
        };
